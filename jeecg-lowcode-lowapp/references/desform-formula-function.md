...
> **注意：** CUSTOM 模式仅支持上述 9 个函数和四则运算符（设计器「选择公式」下拉即这 9 个），不提供 IF、AND、OR、CONCAT 等其他函数入口。即使手工敲入这些函数名，底层执行器虽能识别并返回文本/布尔结果，但文本结果无法继续参与数值运算，最终报 `[公式报错]`，实际不可用。完整的 IF/CONCAT 等函数库适用于**默认值「函数计算」**与**自定义 JS 函数模式**（详见 `desform-default-value.md`），不能用于公式控件的 CUSTOM 模式。
...
> ⚠️ **2026-09-10 实测：`FORMULA` 工厂的 mode/date_begin 等关键字参数会被静默丢弃，返回控件 `options={}`（与 LINK_RECORD filters 同款白名单坑，无警告）。下方 Python 示例的参数**实际上不会生效**。正确做法：`FORMULA('名称')` + `add_widget` 先落位，再 `update_widget(code, {'options': {完整 options 结构}}, key=...)` 补齐，并 `query_form` 回查 mode/type/dateBegin 三键。
...
> **可用语境（2026-09-07 源码核对）：** 以下**全量函数库**（数学/日期/文本/逻辑/高级 5 类）是**默认值「函数计算」引擎**（`advancedSetting.defaultValue.type='function'`）可用的完整集合——表达式按 **JavaScript 语义**异步求值（函数参数可异步解析、支持嵌套/比较运算/字符串拼接）。formula 公式控件（type=number 的 CUSTOM 模式）设计器**只开放上述 9 个数学函数**，其余函数不适用；type=date 的公式控件走预定义 mode，不开放函数库。
...
| `WEEKDAY(date)` | 星期几（中文，`星期日`~`星期六`） | `WEEKDAY('2026-07-29')` → `星期三` |
...
| `MONTHLASTDAY([参考日期])` | 月份最后一天 | `MONTHLASTDAY('2024-02-10')` → `2024-02-29` |

> `WEEKDAY` 参数为空时原样返回空；`MONTHLASTDAY` 参考日期可省（默认当月），输出 `YYYY-MM-DD`，传了非日期值会报错。`DATENOW()` 不传格式默认 `YYYY-MM-DD HH:mm:ss`；若所在控件是 date 且不传格式，则自动用该 date 控件的 `options.format`（yyyy→YYYY、dd→DD 转换）。
...
| 函数 | 说明 |
|------|------|
| `LINKAGET(config, queryItems)` | 从其他工作表查询数据并回填当前字段 |

> **注意：** 函数计算弹窗（FunctionDialog）左侧函数树共 5 类：数学函数/日期函数/文本函数/逻辑函数/高级函数；顶部「常用函数」快捷组为 `IF/CONCAT/AVERAGE/SUM/DATEADD/DATEIF/LINKAGET`。LINKAGET 是默认值第 3 种方式「查询工作表」的函数形态，不用于 formula 公式控件。

**函数文本形态（编辑器内联标签）：**

```
LINKAGET({"appId":"...","desformCode":"target_code","matchType":"AND",
          "operation":"FIRST","rules":[...],"linkages":[...],"sorts":[...]} ,  [条件items])
```
...
7. **可引用字段有类型白名单**（设计器选择字段下拉的过滤规则）：数字计算只能选 `number/integer/money/formula(自身 type=number)/summary/link-field(saveType=save)`；日期计算只能选 `date/formula(自身 type=date)/link-field`；dateAddExp 数值字段只接受 `number/integer/money`
8. 公式字段被其他公式引用时必须 type 一致，**但时长型公式（DATEIF/NOW_DATEIF/PAST_NOW_DATEIF，数值结果）除外**，可被数字型公式引用（QQYUN-12447：引用时自动 parseFloat 其文本结果）
