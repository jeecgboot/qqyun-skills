...
所有其他控件，包括：
`input`, `textarea`, `number`, `integer`, `money`, `radio`, `checkbox`, `select`, `date`, `time`, `switch`, `rate`, `color`, `slider`, `phone`, `email`, `imgupload`, `file-upload`, `buttons`, `text`, `area-linkage`, `location`, `capital-money`, `barcode`, `text-compose`, `auto-number`, `formula`, `hand-sign`, `ocr`, `link-record`（showMode="single" 且 showType 非 table）, `link-field`, `summary`, `select-user`, `select-depart`, `select-depart-post`, `org-role`
...
### 关联组件
`link-record`, `link-field`, `sub-table-design`, `summary`

⛔ 敲敲云禁用：`table-dict`、`select-tree`、`category-linkage`、全部 OA 组件
