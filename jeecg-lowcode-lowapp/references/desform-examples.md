...
create_form('客户信息', 'customer_info', [
    ...
    AREA('所在地区'),  # 默认 areaLevel=3 省市区；level=1 仅省，level=2 省市
...
# ⚠️ 此示例仅在 create_form 路径下安全；手工插已有表 design['list'] 时
#    make_sub_table 返回三元组且 columns[].list 内是未解包子三元组，必须逐层取 [0]，
#    否则保存照报成功、之后 query NPE 整表查不出（详见 desform-sub-table-types.md「Python 用法」警告）
```
